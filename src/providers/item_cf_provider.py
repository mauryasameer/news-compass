...
class ItemCFProvider(RecommenderProvider):
    """Item-based collaborative filtering via cosine item-item similarity."""
    ...
    def fit(self, ratings: pd.DataFrame) -> None:
        ...
        self._matrix = matrix
        self._item_similarity = 1 - pairwise_distances(matrix.T, metric="cosine")

    def recommend(self, user_id: int, k: int, exclude_seen: bool = True) -> list[tuple[int, float]]:
        if self._ratings is None or self._matrix is None or self._item_similarity is None:
            raise RuntimeError("ItemCFProvider.recommend() called before fit()")
        ...
        user_vector = self._matrix[user_id]
        scores = user_vector @ self._item_similarity
